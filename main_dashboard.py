...
with tab1:
    ...
    if not filtered_df.empty:
        ...
        st.warning(
            "No items match your current filter criteria. Please adjust your filters."
        )
...
with tab4:
    ...
    if not filtered_df.empty:
        ...
        with st.spinner("📊 Calculating insights..."):
            over_estimate = filtered_df[
                filtered_df["Sold Price"] > filtered_df["Estimate Avg"]
            ]
            under_estimate = filtered_df[
                filtered_df["Sold Price"] < filtered_df["Estimate Avg"]
            ]
            over_percentage = (len(over_estimate) / len(filtered_df)) * 100
            avg_premium = (
                (
                    (over_estimate["Sold Price"] - over_estimate["Estimate Avg"])
                    / over_estimate["Estimate Avg"]
                    * 100
                ).mean()
        ...
        with col1:
            st.metric(
                "Above Estimate",
                f"{over_percentage:.0f}%",
                f"{len(over_estimate)} items",
            )
        ...
        with col3:
            ...
            st.metric(
                "Top Category",
                highest_category,
                f"{filtered_df['Category'].value_counts().iloc[0]} items",
            )

        st.markdown("---")

        insight_tab1, insight_tab2, insight_tab3 = st.tabs(
            ["📊 Market Performance", "🖼️ Top Items", "📈 Price Patterns"]
        )

        with insight_tab1:
            st.markdown("#### Estimate vs Actual Performance")
            st.markdown(
                "How auction estimates compared to final sale prices for the most valuable items."
            )

            with st.spinner("📊 Creating performance analysis..."):
                fig_dumbbell = create_dumbbell_chart(filtered_df, n_items=10)
                st.plotly_chart(
                    fig_dumbbell, use_container_width=True, key="dumbbell_insights"
                )
        ...
        with insight_tab2:
            ...
            st.markdown(
                "The most valuable and distinctive pieces from the filtered selection."
            )
            ...
            with col_right:
                ...
                with st.spinner("💰 Finding best values..."):
                    fig_cheapest = create_horizontal_bar_chart(
                        filtered_df, "Top 10 Cheapest", n_items=10, ascending=True
                    )
                    st.plotly_chart(
                        fig_cheapest, use_container_width=True, key="cheapest_insights"
                    )
    else:
