...
def create_sidebar_filters(df):
    ...
    other_categories = [
        ...
        if cat not in creative_categories + equipment_categories + personal_categories
    ]
    ...
    selected_categories = [
        c for c, checked in st.session_state.selected_categories.items() if checked
    ]
    ...
    if (
        st.session_state.get("price_filter")
        or keyword
        or len(
            [
                c
                for c, checked in st.session_state.selected_categories.items()
                if checked
            ]
        )
        < len(categories)
...
def create_data_table(df):
    ...
    gb.configure_column(
        "Sold Price", type=["numericColumn"], valueFormatter=dollar_formatter
    )
    gb.configure_column(
        "Estimate Avg", type=["numericColumn"], valueFormatter=dollar_formatter
    )

    # Column width configuration
    gb.configure_column("Image", minWidth=120, maxWidth=150)
    gb.configure_column(
        "Title", minWidth=250, maxWidth=450, wrapText=True, autoHeight=True
    )
    ...
    try:
        with st.spinner(
            f"📊 Loading interactive table with {len(df_display)} items..."
        ):
            AgGrid(
                df_display[
                    [
        ...
        st.dataframe(
            df_display[["Title", "Sold Price", "Estimated Price", "Category"]],
...
def create_download_buttons(df):
    ...
    with col2:
        # HTML table download
        df_export = df.copy()
        df_export["Link"] = df_export["URL"].apply(
            lambda x: f'<a href="{x}" target="_blank">Open</a>'
        )
        html_table = df_export[
            [
                "Title",
                "Sold Price",
                "Estimated Price",
